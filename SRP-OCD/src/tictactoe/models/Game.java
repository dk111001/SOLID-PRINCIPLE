...
import tictactoe.startegies.GameWinningStrategy.GameWinningStrategy;
import tictactoe.startegies.GameWinningStrategy.OrderOneGameWinningStrategy;
...
public class Game {
    ...
    public void makeNextMove() {
        ...
        Move move = toMovePlayer.decideMove();
    ...
    public static class Builder {
        ...
        private boolean valid() throws InvalidGameConstructionParametersException {
            ...
            HashSet<Character> playerSymbols = new HashSet<Character>();
            for(Player player:players){
                char symbol = player.getSymbol();
                if(playerSymbols.contains(symbol)){
                    throw new InvalidGameConstructionParametersException("Two players cannot have same symbol");
                }
            ...
            for(Player player:players){
                if(player.getPlayerType().equals(PlayerType.BOT))
                    botCount++;
                if(botCount>1)
                    throw new InvalidGameConstructionParametersException("Only one bot player is allowed per game");
            }
